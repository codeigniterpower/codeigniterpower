...
class CI_Table {
	...
	function set_datatables($options = array())
	{
		...
		if (count($options) === 0)
		{
			$options = array("searchable" => "true", "fixedHeight" => "true", "perPage" => "15");
		}
	...
	function _compile_datatable()
	{
		...
		{
			// detecting if id might be autogenerated js datatable code
			//if ( strpos($tableopentag, 'vanilladatatable') !== FALSE)
			{
				// got the id for preparing the js code
				$tableid = $this->tableid;
				// lest extrac any custom made from user template
				...
				if( $this->enabledatatable != '0' AND $this->datatableoptions != '' )
				{
					...
					$tablejscodetags .= '
						...
						topContainer.appendChild(topPager);
						// Enable the new pager
						topContainer.addEventListener("click", function(e) {	var target'.$tableid.' = e.target;if ( target'.$tableid.'.nodeName.toLowerCase() === \'a\' && target'.$tableid.'.hasAttribute("data-page") ) {		var page = target'.$tableid.'.getAttribute("data-page");		data'.$tableid.'.page(page);	}	}	);
